...
"""
NOTES: When specifying testing data, we want to use the training data as the "normal" distribution of observations, and
"compare" the testing distribution to the norm. One potential concern: in the case where the entire set of testing data
is abnormal, and both the train and test data are the same length, then the test data would mask the distribution of
the normal data (worded horribly but will fix later). Solution: set a ratio of some kind, say 80% training data and 20%
testing data.

Problem. Impractical in practice. Our passes sometimes include 40k points, which means we would need way more points
